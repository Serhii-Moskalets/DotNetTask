...
public class SecurityTokenTests
{
    ...
    public void Create_Should_InitializeProperties_When_DataIsValid()
    {
        ...
        // Act
        SecurityToken token = SecurityToken.Create(ValidTokenValue, this._validDuration, type, this._currentTime, metadata);
    ...
    {
        // Arrange
        TimeSpan invalidDuration = TimeSpan.FromSeconds(seconds);
    ...
    {
        // Arrange
        SecurityToken token = SecurityToken.Create(ValidTokenValue, this._validDuration, UserTokenType.PasswordReset, this._currentTime);
    ...
    public void IsValid_Should_ReturnFalse_When_ConditionsNotMet(string providedValue, UserTokenType expectedType, int minutesToAdd)
    {
        // Arrange
        SecurityToken token = SecurityToken.Create(ValidTokenValue, this._validDuration, UserTokenType.PasswordReset, this._currentTime);
        DateTime testTime = this._currentTime.AddMinutes(minutesToAdd);
